<template>
  <div class="post-detail">
    ...
    <div v-if="!post" class="detail-empty">
      <p>动态不存在或已被删除</p>
    ...
    <div v-else class="detail-content">
      <div class="detail-author">
        ...
        <div class="detail-author-info">
          <div class="detail-name" :class="{'sage-author-name': isSage, 'philosopher-link': isSage}" @click.stop="isSage && $emit('open-philosopher', post.authorId || post.authorName)">{{ displayNickname }}</div>
          <div class="detail-time" @click="showFullTime = !showFullTime">{{ showFullTime ? formatFullTime(post.createdAt) : formatTime(post.createdAt) }}<span v-if="post.updatedAt && post.updatedAt > post.createdAt" class="edited-badge"> · 已编辑</span></div>
        </div>
      ...
      <div v-if="post.content" class="detail-body">{{ post.content }}</div>

      <div v-if="post.location && post.location.trim()" class="detail-location">
        <span class="location-icon">📍</span>
        <span class="location-text">{{ post.location.trim() }}</span>
      </div>

      <div v-if="post.tags && post.tags.length" class="detail-tags">
        <span v-for="tag in post.tags" :key="tag" class="post-tag-chip" @click.stop="handleTagClick(tag)">#{{ tag }}</span>
      </div>

      <ImageGrid v-if="post.images && post.images.length" :images="post.images" @open-preview="$emit('open-image-preview', $event)" />

      <MusicCard v-if="post.music && post.music.title" :music="post.music" />

      <div class="detail-actions">
        <button class="action-btn" :class="{ liked: userLiked }" @click="$emit('toggle-like', post.id)">
          {{ userLiked ? '♥ 已赞' : '♡ 赞' }}
          <span v-if="post.likeCount">({{ post.likeCount }})</span>
        </button>
        <button class="action-btn" @click="focusComment">评论<span v-if="post.comments && post.comments.length">({{ post.comments.length }})</span></button>
      </div>

      <div v-if="hasInteraction" class="interaction-area">
        ...
        <div v-if="hasComments" class="comment-section">
          ...
          <template v-for="c in (post.comments || [])" :key="c.id">
            ...
            <div v-if="c.isAi && !c.loading" class="ai-reply-action">
              <button v-if="aiReplyCount(c) < 3" class="reply-ai-btn" @click="openReplyInput(c.id)">回复先贤</button>
              <span v-else class="reply-limit">对话已满</span>
            </div>
            <div v-if="activeReplyId === c.id" class="reply-input-row">
              <input v-model="replyText" class="reply-input" placeholder="回复这位先贤…" @keyup.enter="sendReply(c)" />
      ...
      <div class="detail-comment-input">
        <input ref="commentInputRef" v-model="commentText" class="comment-input" placeholder="写下你的回应…" @keyup.enter="sendComment" />
...
const emit = defineEmits(['back', 'toggle-like', 'add-comment', 'delete-post', 'delete-comment', 'reply-to-ai', 'open-philosopher', 'open-image-preview', 'edit-post', 'filter-by-tag'])
...
const displayNickname = computed(() => getPostAuthorName(props.post, props.profile))
...
const sageAvatarText = computed(() => getSageAvatarText(getPostAuthorName(props.post, props.profile)))
const userLiked = computed(() =>
  Array.isArray(props.post.likedBy)
  && props.post.likedBy.some(item => item?.id === 'user')
)

const canEdit = computed(() => !props.post.isDailySage && props.post.authorType !== 'sage')
const hasComments = computed(() => props.post && props.post.comments && props.post.comments.length)
const hasInteraction = computed(() => (props.post.likedBy && props.post.likedBy.length) || hasComments.value)
...
function handleDeletePost() {
  showMenu.value = false
  console.log("[delete] detail button clicked:", props.post.id)
...
<style scoped>
.post-detail { background-color: #fff; min-height: 100vh; }
...
.nav-back {
  background: none; border: none; font-size: 24px; color: #576B95;
...
.detail-empty {
  ...
  justify-content: center; padding: 120px 16px 0;
}
.detail-empty p { font-size: 15px; color: #888; margin-bottom: 20px; }
.back-btn {
  background-color: #07C160; color: #fff; border: none;
  border-radius: 8px; padding: 10px 32px; font-size: 15px;
  cursor: pointer;
}

.detail-content { padding: 16px; }

.detail-author { display: flex; align-items: center; margin-bottom: 14px; }
.detail-avatar { flex-shrink: 0; margin-right: 10px; }
...
.detail-author-info { display: flex; flex-direction: column; flex: 1; }
.detail-name { font-size: 15px; font-weight: 600; color: #576b95; line-height: 1.3; }
.detail-time { font-size: 12px; color: #888; margin-top: 2px; cursor: pointer; }
.detail-time:hover { color: #576B95; }
.sage-author-name { color: #8A6A2F; font-weight: 600; }
.edited-badge { font-size: 12px; color: #999; }

.detail-more-wrap { position: relative; margin-left: 8px; }
.more-btn {
...
.detail-body {
  ...
  word-break: break-word; white-space: pre-wrap; margin-bottom: 10px;
}

.detail-location {
  display: inline-flex; align-items: center; gap: 4px;
  margin-bottom: 10px; font-size: 13px; color: #576B95;
}
.location-icon { flex-shrink: 0; }
.location-text { overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }

.detail-actions {
  display: flex; justify-content: flex-end; gap: 20px;
  padding: 10px 0; border-top: 1px solid #EAEAEA; margin-top: 10px;
}
.action-btn {
...
.comment-divider { height: 1px; background-color: #EAEAEA; margin: 0 -10px 8px; }
.comment-item {
...
.replies-area {
  margin: 2px 0 4px 16px; padding-left: 10px;
  border-left: 2px solid #E8DCC8;
}
.reply-item { font-size: 13px; line-height: 1.7; color: #222; padding: 2px 0; }
.reply-item.ai-reply { color: #6B5B3E; }
.reply-user { color: #576b95; font-weight: 500; }
.reply-user.ai-name { color: #C9A96E; }
.reply-to { color: #999; font-size: 12px; }

.ai-reply-action { margin: 4px 0 2px 16px; padding-left: 10px; }
.reply-ai-btn {
  background: none; border: none; font-size: 12px;
...
.philosopher-link { cursor: pointer; transition: opacity 0.2s; }
.philosopher-link:hover { text-decoration: underline; opacity: 0.8; }
